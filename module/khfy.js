...
async function getProductKhfy() {
    try {
        const response = await axios.get(process.env.URL_KHFY2);
...
async function orderProductKhfy(data) {
    try {
        const { code, destination } = data;
        const ref_id = `WZ${generateRandomString(13)}`;

        const targetUrl = `${process.env.URL_KHFY}/trx?produk=${code}&tujuan=${destination}&reff_id=${ref_id}&api_key=${process.env.API_KHFY}`;
